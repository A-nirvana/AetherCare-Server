...
export const notifyMedicalStaff = async (payload: StaffNotificationPayload) => {
  ...
  try {
    ...
    const message = await medicTwilioClient.messages.create({
      body: `Location: ${lastData.data.Latitude},${lastData.data.Longitude} `,
      to: staffPhoneNumber,
      from: medicPhoneNumber || '',
    });

    console.log(`Successfully sent Call and SMS notification to staff at ${staffPhoneNumber}. SID: ${message.sid},${call.sid}`);
  } catch (error: any) {
